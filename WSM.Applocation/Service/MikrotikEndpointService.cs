...
namespace WSM.Application.Service
{
    public class MikrotikEndpointService : IMikrotikEndpointService
    {
        ...
        public async Task<OperationResult<Guid?>> CreateMikrotikEndpoint(MikrotikEndpointCreateDto mikrotikEndpointCreateDto)
        {
            ...
            if (existingMikrotikEndpoint != null)
            {
                ...
                return OperationResult<Guid?>.ErrorResult("MikrotikEndpoint with the same public key already exists.");
            }
            //get User Id
            ...
            if (existUser == null)
            {
                ...
                return OperationResult<Guid?>.ErrorResult("User  does not exist.");
            }
            //get CHR Id
            ...
            if (existMikrotikCHR == null)
            {
                ...
                return OperationResult<Guid?>.ErrorResult("CHR  does not exist.");
            }
            var IPs = await _mikrotikEndpointRepository.GetAvailableIpsAsync("10.10.10.0/24");
            if (IPs == null)
                   {
                _logger.LogInformation($"No available IPs.");
                return OperationResult<Guid?>.ErrorResult("No available IPs.");
            }
            ...
            var response = await _mikrotikApiService.MikrotikSetComand(existMikrotikCHR, "rest/interface/wireguard/peers", wgCreateDtoJson);

            if (response.Data == null)
            {
                _logger.LogInformation("Response data is null.");
                return OperationResult<Guid?>.ErrorResult("Error creating MikrotikEndpoint in Mikrotik CHR: Response data is null.");
            }
            ...
            if (res.Id == null)
            {
                ...
                return OperationResult<Guid?>.ErrorResult("Error creating MikrotikEndpoint in Mikrotik CHR.");
            }
            ...
            if (result == -1)
            {
                ...
                return OperationResult<Guid?>.ErrorResult("Error creating MikrotikEndpoint in the repository.");
            }
            return OperationResult<Guid?>.SuccessResult(mikrotikEndpoint.Id);
        }
        public async Task<OperationResult<MemoryStream?>> GetMikrotikEndpointQrcode(WgReadDto wgReadDto)
        {
            ...
            string config = "[Interface]\n" +
                            ...
                            $"Endpoint = {configClass.ConfigEndPoint}:{configClass.ConfigEndPointPort.ToString()}\n" +
